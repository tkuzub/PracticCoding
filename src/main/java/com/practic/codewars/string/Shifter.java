 ...
 * can be read, although sometimes in a different way. So, uppercase letters "H", "I", "N", "O", "S", "X", "Z" after
 ...
 * After turning the sheet, this word can also be read, although in a different way. So, the word "WOW "turns into the
 * word "MOM". On the other hand, the word "HOME" is not a shifter.
 *
...
public class Shifter {
    public int count(String st){
        return(int) Arrays.stream(st.split("\\s+")).
                distinct().
                filter(s -> Arrays.stream(s.split("")).
                        allMatch(x -> x.matches("[HINOSXZMW]"))).
                count();
    }
}
