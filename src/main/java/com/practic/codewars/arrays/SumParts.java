...
The function parts_sums (or its variants in other languages) will take as parameter a list ls and return a list
...
public class SumParts {
    public static int[] sumParts(int[] ls) {
        int[] array = IntStream.
                range(0, ls.length).
                map(value -> IntStream.
                        range(value, ls.length).
                        map(num -> ls[num]).
                        sum()).
                toArray();
        int[] result = new int[ls.length + 1];
        System.arraycopy(array, 0, result, 0, ls.length);
        return result;
    }
}
